...
int32_t		heap_extract_min(Heap *heap)
{
	...
	int32_t next = heap->data[2];	
	_swap_(&heap->data[1], &heap->data[heap->size]);
	--heap->size;
	_heapify_(heap->data, heap->size, 1);
	if(min<1200){
	printf("event with smallest time stamp: %d AM\n",min);
	}
	if(min>1200){
	printf("event with smallest time stamp: %d PM\n",min);
	}
	if(next<1200){
	printf("Next flight: %d AM\n",next);
	}
	if(next>1200){
	printf("Next flight: %d PM\n",next);
	}
	return min;
}
